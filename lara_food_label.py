#!/usr/bin/env python3
"""Generate a pet food feeding guide label for Zebra GX420d (203dpi, 4x6 portrait)."""
...
def main():
    ...
    name_font = ImageFont.truetype(BOLD_FONT, 120)
    ...
    morning_items = ["1 scoop dry food"]
    evening_items = [
        "1 scoop dry food",
        "1 orange fish oil",
        "1 white joint supplement",
        "Top with glob of wet food",
    ]
    note = "*She may not eat until after a walk"

    # --- Measure total content height with a dummy draw ---
    def draw_content(draw, y_start):
        y = y_start

        # Name
        bbox = draw.textbbox((0, 0), "LARA", font=name_font)
        name_w = bbox[2] - bbox[0]
        draw.text((x_left + (content_width - name_w) // 2, y), "LARA", fill="black", font=name_font)
        y = draw.textbbox((0, y), "LARA", font=name_font)[3] + 12
        ...
        y = draw.textbbox((0, y), subtitle, font=subtitle_font)[3] + 24
        ...
        # Evening
        draw.text((x_left, y), "Evening", fill="black", font=section_font)
        y = draw.textbbox((x_left, y), "Evening", font=section_font)[3] + 18
    ...
    # --- Save ---
    output_png = "lara-food-preview.png"
    output_zpl = "lara-food.zpl"
